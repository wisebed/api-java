
package eu.wisebed.api.snaa;

import java.net.MalformedURLException;
import java.net.URL;
import java.util.logging.Logger;
import javax.xml.namespace.QName;
import javax.xml.ws.Service;
import javax.xml.ws.WebEndpoint;
import javax.xml.ws.WebServiceClient;
import javax.xml.ws.WebServiceFeature;


/**
 * This class was generated by the JAX-WS RI.
 * JAX-WS RI 2.1.6
 * Generated source version: 2.1
 * 
 */
@WebServiceClient(name = "SNAAService", targetNamespace = "http://testbed.wisebed.eu/api/snaa/v1/", wsdlLocation = "REPLACE_WITH_ACTUAL_URL")
public class SNAAService
    extends Service
{

    private final static URL SNAASERVICE_WSDL_LOCATION;
    private final static Logger logger = Logger.getLogger(eu.wisebed.api.snaa.SNAAService.class.getName());

    static {
        URL url = null;
        try {
            URL baseUrl;
            baseUrl = eu.wisebed.api.snaa.SNAAService.class.getResource(".");
            url = new URL(baseUrl, "REPLACE_WITH_ACTUAL_URL");
        } catch (MalformedURLException e) {
            logger.warning("Failed to create URL for the wsdl Location: 'REPLACE_WITH_ACTUAL_URL', retrying as a local file");
            logger.warning(e.getMessage());
        }
        SNAASERVICE_WSDL_LOCATION = url;
    }

    public SNAAService(URL wsdlLocation, QName serviceName) {
        super(wsdlLocation, serviceName);
    }

    public SNAAService() {
        super(SNAASERVICE_WSDL_LOCATION, new QName("http://testbed.wisebed.eu/api/snaa/v1/", "SNAAService"));
    }

    /**
     * 
     * @return
     *     returns SNAA
     */
    @WebEndpoint(name = "SNAAPort")
    public SNAA getSNAAPort() {
        return super.getPort(new QName("http://testbed.wisebed.eu/api/snaa/v1/", "SNAAPort"), SNAA.class);
    }

    /**
     * 
     * @param features
     *     A list of {@link javax.xml.ws.WebServiceFeature} to configure on the proxy.  Supported features not in the <code>features</code> parameter will have their default values.
     * @return
     *     returns SNAA
     */
    @WebEndpoint(name = "SNAAPort")
    public SNAA getSNAAPort(WebServiceFeature... features) {
        return super.getPort(new QName("http://testbed.wisebed.eu/api/snaa/v1/", "SNAAPort"), SNAA.class, features);
    }

}
